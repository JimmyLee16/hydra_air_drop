...
*Tích hợp Cardano Stake Pool để tăng tính minh bạch và khuyến khích dài hạn*
...
```math
Donate_Amount = (Stake_Reward × Donate_Rate) + Fixed_Bonus
```
...
sequenceDiagram
    ...
    Wallet->>StakePool: Delegation
    Note over StakePool: Mỗi Epoch (5 ngày)
    StakePool->>Wallet: Stake Reward
    Wallet->>GameContract: Auto-Donate (Smart Contract)
    GameContract->>PrizePool: +Donate_Amount
    GameContract->>Wallet: +NFT/Token Bonus
```
